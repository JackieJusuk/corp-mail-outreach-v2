...
로컬 Claude Code가 (Routine 등으로) 5분 간격 근처에 이 스크립트를 실행해
...
발송 자격 조건 (모두 만족해야 함):
    ...
    - 가장 최근 성공 발송으로부터 5분 이상 경과 (5분당 1건 페이싱)
...
MIN_INTERVAL_SECONDS = 300  # 5분당 1건

# 2026년 대한민국 법정공휴일 (관공서의 공휴일에 관한 규정, 대체공휴일 포함, 총 19일).
# 외부 API 대신 하드코딩: 이 스크립트는 스케줄러가 무인으로 실행하므로 네트워크
# 의존성을 늘리지 않기 위함. 연도가 바뀌면 다음 해 목록으로 갱신 필요.
KOREAN_HOLIDAYS = {
    "2026-01-01",  # 신정
    "2026-02-16", "2026-02-17", "2026-02-18",  # 설날 연휴
    "2026-03-01", "2026-03-02",  # 삼일절 (3/2 대체공휴일)
    "2026-05-05",  # 어린이날
    "2026-05-24", "2026-05-25",  # 부처님오신날 (5/25 대체공휴일)
    "2026-06-06",  # 현충일
    "2026-08-15", "2026-08-17",  # 광복절 (8/17 대체공휴일)
    "2026-09-24", "2026-09-25", "2026-09-26",  # 추석 연휴
    "2026-10-03", "2026-10-05",  # 개천절 (10/5 대체공휴일)
    "2026-10-09",  # 한글날
    "2026-12-25",  # 크리스마스
}
...
def main():
    ...
    if today_str in KOREAN_HOLIDAYS:
        emit({"eligible": False, "reason": f"법정공휴일({today_str})에는 발송할 수 없습니다."})
        return
    ...
    # 페이싱 확인: 마지막 성공 발송으로부터 5분이 지나야 함
    ...
    if last:
        ...
        if elapsed < MIN_INTERVAL_SECONDS:
            ...
            emit({
                "eligible": False,
                "reason": f"마지막 발송 후 5분이 지나지 않았습니다. 약 {wait_sec}초 후 다시 확인하세요.",
            })
            conn.close()
            return
